...
Since it requires several steps, using Farms with ChampagneSwap can seem intimidating at first. This guide will walk you through using the Farms contract directly through BscScan.

{% hint style="warning" %}
Please understand that using BscScan to interact with contracts is not recommended for beginners. If you're not feeling confident, we suggest using the [How to Use Farms guide](https://docs.panchamswap.finance/products/yield-farming/how-to-use-farms) instead.
{% endhint %}
...
1\. Open the [ChampagneSwap website’s Farms code on GitHub](https://github.com/panchamswap/pancham-frontend/blob/master/src/config/constants/farms.ts).

2\. **Control**/**command** + **F** and search for your pair by ticker (not project name). For example, 'CHAM-BUSD'.
...
But assuming you'd like to confirm that, visit the [ChampagneSwap: Main Staking Contract BscScan page](https://bscscan.com/address/0x73feaa1ee314f8c655e354234017be2193c9e24e#writeContract). You'll see the address in the top-left. Click the **pages icon** to copy this to clipboard. You'll need it soon.
...
1\. First, open [farms.ts on GitHub](https://github.com/panchamswap/pancham-frontend/blob/master/src/config/constants/farms.ts).

2\. **Control**/**command** + **F** and search for your pair by ticker (not project name). For example, 'CHAM-BNB'
...
1\. First, visit the [ChampagneSwap Farms page](https://panchamswap.finance/farms) and search for your chosen pair using the "SEARCH" field in the top right. We're using CHAM-BUSD for this example.
...
5\. You’re also going to need to approve the amount of LP Tokens the contract can spend. In the value field, you’ll need to enter the amount in Wei. You can use the [BscScan Unit Converter](https://www.bscscan.com/unitconverter) to easily change your amount into Wei. Here we'll use 5 CHAM-BUSD LP Tokens.
...
1\. Back on the [ChampagneSwap: Main Staking Contract BscScan page](https://bscscan.com/address/0x73feaa1ee314f8c655e354234017be2193c9e24e#writeContract), go to **Contract**, and then **Write Contract**.
...
{% hint style="danger" %}
**Using the emergency withdraw function will forfeit your CHAM rewards!**

The ChampagneSwap team strongly suggests avoiding this function unless advised to do so officially by the ChampagneSwap team, or if you are very comfortable interacting with smart contracts and understand the underlying code.
{% endhint %}

‌1. On the [ChampagneSwap: Main Staking Contract BscScan page](https://bscscan.com/address/0x73feaa1ee314f8c655e354234017be2193c9e24e#writeContract), go to **Contract**, and then **Write Contract**.
